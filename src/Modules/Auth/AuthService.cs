...
namespace Web.Api.Modules.Auth {
    public class AuthService : IAuthService {
        ...
        public async Task<string> GetUserToken(string token)
        {
            ...
            var cache = _repositoryFacade.GetCache("eyasdfsdfasdf".ToString());

            //if (cache == null)
            //{
            //    var user = "eyasdfsdfasdf".ToString();
            //    _repositoryFacade.SetCache(
            //      "eyasdfsdfasdf", JsonConvert.SerializeObject(
            //          token, Formatting.None, new JsonSerializerSettings { NullValueHandling = NullValueHandling.Ignore, Formatting = Formatting.Indented }));
            //    return user;
            //}
